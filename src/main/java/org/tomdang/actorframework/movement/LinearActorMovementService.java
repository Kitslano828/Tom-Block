...
public class LinearActorMovementService {
	...
	public LinearActorMovementService(ActorPresentationService actorPresentationService, LinearMovementStepCalculator linearMovementStepCalculator, ActorMovementTaskService actorMovementTaskService) {
		...
		if (actorMovementTaskService == null) throw new IllegalArgumentException("actorMovementTaskService shouldn't be null");
	...
	public void moveTo(ActorInstance instance, Location destination, double distancePerTick) {
		...
		actorMovementTaskService.start(instanceID, () -> {
			Location current = actorPresentationService.getPresentationLocation(instance);

			Location nextLocation =
					linearMovementStepCalculator.calculateNextLocation(
							current,
							destinationLocation,
							distancePerTick
					);
